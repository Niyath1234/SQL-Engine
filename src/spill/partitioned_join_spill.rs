/// PartitionedJoinSpill: Spill support for hash joins
/// 
/// This module provides spill functionality for hash join operators,
/// implementing Grace hash join algorithm with partition-based spilling.
...
/// Partitioned join spill manager
pub struct PartitionedJoinSpill {
    /// Spill directory
    spill_dir: PathBuf,
    ...
    /// Build-side partitions (spilled to disk)
    build_partitions: Vec<Vec<PathBuf>>,
    
    /// Probe-side partitions (spilled to disk)
    probe_partitions: Vec<Vec<PathBuf>>,
    
    /// Current partition being processed
    current_partition: usize,
}

impl PartitionedJoinSpill {
    /// Create a new partitioned join spill manager
    pub fn new(spill_dir: PathBuf, partition_count: usize) -> Self {
    ...
    /// Spill a build-side batch to a partition
    pub fn spill_build_batch(
        &mut self,
        batch: &VectorBatch,
        partition_idx: usize,
    ) -> EngineResult<PathBuf> {
        let file_path = self.get_build_partition_file(partition_idx);
        ...
        self.build_partitions[partition_idx].push(file_path.clone());
        
        debug!(
            partition = partition_idx,
            file = ?file_path,
            rows = batch.row_count,
            "Spilled build batch to partition"
        );
        
        Ok(file_path)
    }
    
    /// Spill a probe-side batch to a partition
    pub fn spill_probe_batch(
        &mut self,
        batch: &VectorBatch,
        partition_idx: usize,
    ) -> EngineResult<PathBuf> {
        let file_path = self.get_probe_partition_file(partition_idx);
        ...
        // Track partition file
        self.probe_partitions[partition_idx].push(file_path.clone());
        
        debug!(
            partition = partition_idx,
            file = ?file_path,
            rows = batch.row_count,
            "Spilled probe batch to partition"
        );
        
        Ok(file_path)
    }
    
    /// Load a build partition from disk
    pub fn load_build_partition(
        &self,
        partition_idx: usize,
    ) -> EngineResult<Vec<VectorBatch>> {
        let mut batches = Vec::new();
        
        for file_path in &self.build_partitions[partition_idx] {
        ...
        info!(
            ...
            "Loaded build partition from disk"
        );
        
        Ok(batches)
    }
    
    /// Load a probe partition from disk
    pub fn load_probe_partition(
        &self,
        partition_idx: usize,
    ) -> EngineResult<Vec<VectorBatch>> {
        let mut batches = Vec::new();
        
        for file_path in &self.probe_partitions[partition_idx] {
            let batch = self.load_batch_from_file(file_path)?;
            batches.push(batch);
        }
        ...
        info!(
            partition = partition_idx,
            batch_count = batches.len(),
            "Loaded probe partition from disk"
        );
    ...
    /// Get build partition file path
    fn get_build_partition_file(&self, partition_idx: usize) -> PathBuf {
        let file_name = format!("build_partition_{}.arrow", partition_idx);
        self.spill_dir.join(file_name)
    }
    
    /// Get probe partition file path
    fn get_probe_partition_file(&self, partition_idx: usize) -> PathBuf {
        let file_name = format!("probe_partition_{}.arrow", partition_idx);
        self.spill_dir.join(file_name)
    }
    ...
    pub fn cleanup(&self) -> EngineResult<()> {
        for partition_files in &self.build_partitions {
        ...
        for partition_files in &self.probe_partitions {
            for file_path in partition_files {
                if let Err(e) = std::fs::remove_file(file_path) {
                    warn!("Failed to remove spill file {:?}: {}", file_path, e);
                }
